...
export default function MyOrdersPage() {
  ...
  // This state is to force a re-render when an order is cancelled
  ...
  const ordersRef = useMemoFirebase(() => user ? collection(firestore, 'users', user.uid, 'orders') : null, [firestore, user, version]);
                      ...
                      <div className="space-y-4">
                       ...
                       {order.status === 'Placed' && (
                         <CardFooter className="px-0 pt-6 justify-end">
                            <CancelOrderDialog orderId={order.id} userId={user.uid} onCancelSuccess={forceReRender}/>
                         </CardFooter>
                       )}
